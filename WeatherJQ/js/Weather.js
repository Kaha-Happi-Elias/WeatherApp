...
const $loadingIndicator = $('<div class = "spinner-border position-absolute top-50 start-50"></div>').css({"width": "3rem", "height": "3rem"}).hide().appendTo('body');
/*const $loadingIndicator = $('<div id="loader"></div>').hide().appendTo('body');*/
...
const cityForm = $('#city');
const reportSection = $('#wheather-report');

cityForm.submit(function(event){
    event.preventDefault();
    const chosenCity = citySelect.val();
    showLoading();
    ...
    $.ajax({
        ...
        success: function(weatherData) {
            const weatherReport = "The weather in " + weatherData.name + " is " + weatherData.weather[0].main + " with a temperature of " + weatherData.main.temp + "°C.";
            reportSection.html(weatherReport);
        },
        error: function(error) {
            if (error.status  === 404){
                reportSection.text('City not found!');
            } else {
                const errorMessage = "Error fetching weather data: " + error.statusText;
                reportSection.text(errorMessage);
...
const $stateSelect = $('.state');
const $citySelect = $('.city');
const $cityForm = $('#city-form'); // Added to consistently use jQuery selectors
const $reportSection = $('#weather-report'); // Corrected typo here; should be 'weather-report'
